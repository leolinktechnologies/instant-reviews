...
export default function BusinessReviewClient({ slug, businessData }: ClientProps) {
  ...
  // ⚡ Fire-and-Forget Analytics Logger (WITHOUT AWAIT)
  const logAnalytics = (
    eventType: 'visited' | 'rated' | 'generated' | 'copied_redirect',
    ratingVal?: number | null
  ) => {
    if (!slug) return;
    // Background execution - main UI thread block nahi hoga
    supabase
  ...
  // Page Load / Visited Event & Prefetching
  useEffect(() => {
    // ⚡ Background analytics log (bina await ke)
